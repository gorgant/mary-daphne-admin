...
const executeActions = async (order: Order) => {
  ...
  const orderFbRes = await db.collection(AdminCollectionPaths.ORDERS).doc(order.id).set(order)
    .catch(err => {functions.logger.log(`Failed to store order in admin database:`, err); throw new functions.https.HttpsError('internal', err);});
  functions.logger.log('Order stored in admin database:', orderFbRes);
  ...
  const subOrderFbRes = await db.collection(AdminCollectionPaths.SUBSCRIBERS).doc(order.email)
    ...
    .set(order)
    .catch(err => {functions.logger.log(`Failed to update subscriber order data in admin database:`, err); throw new functions.https.HttpsError('internal', err);});
  functions.logger.log('Subscriber updated with order data', subOrderFbRes);  
...
export const storeOrder = functions.pubsub.topic(AdminTopicNames.SAVE_ORDER_TOPIC).onPublish( async (message, context) => {
  ...
  functions.logger.log('Store order request received with this data:', order);
